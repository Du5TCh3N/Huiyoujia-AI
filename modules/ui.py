            ...
            with gr.Column(elem_id=f"{id_part}_prompt_container", scale=6):
                with gr.Row():
                    with gr.Column(scale=80):
                        with gr.Row():
                            self.prompt = gr.Textbox(label="请输入生成文案 Prompt", elem_id=f"{id_part}_prompt", show_label=False, lines=3, placeholder="请输入生成文案 Prompt (press Ctrl+Enter or Alt+Enter to generate)", elem_classes=["prompt"])
                            self.prompt_img = gr.File(label="", elem_id=f"{id_part}_prompt_image", file_count="single", type="binary", visible=False)

                with gr.Row():
                    with gr.Column(scale=80):
                        with gr.Row():
                            self.negative_prompt = gr.Textbox(label="请输入负向词 Negative prompt", elem_id=f"{id_part}_neg_prompt", show_label=False, lines=3, placeholder="请输入负向词 Negative prompt (press Ctrl+Enter or Alt+Enter to generate)", elem_classes=["prompt"])
...
def create_sampler_and_steps_selection(choices, tabname):
    if opts.samplers_in_dropdown:
        with FormRow(elem_id=f"sampler_selection_{tabname}"):
            sampler_name = gr.Dropdown(label='采样模式 Sampling method', elem_id=f"{tabname}_sampling", choices=choices, value=choices[0])
            steps = gr.Slider(minimum=1, maximum=50, step=1, elem_id=f"{tabname}_steps", label="采样步数 Sampling steps", value=20)
    else:
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        with gr.Tab("生成 Generation", id="txt2img_generation") as txt2img_generation_tab, ResizeHandleRow(equal_height=False):
                ...
                for category in ordered_ui_categories():
                    if category == "sampler":
                        ...
                        with FormRow():
                            ...
                            if opts.dimensions_and_batch_together:
                                with gr.Column(elem_id="txt2img_column_batch", visible=False):
                                    batch_count = gr.Slider(minimum=1, step=1, label='出图次数 Batch count', value=1, elem_id="txt2img_batch_count")
                                    batch_size = gr.Slider(minimum=1, maximum=8, step=1, label='出图张数 Batch size', value=1, elem_id="txt2img_batch_size")
                        ...
                        with gr.Row(elem_id="txt2img_accordions", elem_classes="accordions"):
                            with InputAccordion(False, label="高清精绘 Hires. fix", elem_id="txt2img_hr") as enable_hr:
                                with enable_hr.extra():
                                    hr_final_resolution = FormHTML(value="", elem_id="txtimg_hr_finalres", label="精绘分辨率 Upscaled resolution", interactive=False, min_width=0)

                                with FormRow(elem_id="txt2img_hires_fix_row1", variant="compact"):
                                    hr_upscaler = gr.Dropdown(label="精绘方式 Upscaler", elem_id="txt2img_hr_upscaler", choices=[*shared.latent_upscale_modes, *[x.name for x in shared.sd_upscalers]], value=shared.latent_upscale_default_mode)
                                    hr_second_pass_steps = gr.Slider(minimum=0, maximum=150, step=1, label='精绘步数 Hires steps', value=0, elem_id="txt2img_hires_steps")
                                    denoising_strength = gr.Slider(minimum=0.0, maximum=1.0, step=0.01, label='去噪强度 Denoising strength', value=0.7, elem_id="txt2img_denoising_strength")
    ...
    interfaces = [
        ...
        (extras_interface, "后期处理 Extras", "extras"),
        (pnginfo_interface, "图片信息 PNG Info", "pnginfo"),
        (modelmerger_ui.blocks, "模型融合 Checkpoint Merger", "modelmerger"),
        (train_interface, "训练 Train", "train"),
    ]

    interfaces += script_callbacks.ui_tabs_callback()
    interfaces += [(settings.interface, "系统信息 Settings", "settings")]
    ...
    theme = gr.themes.Base(
        ...
        body_text_color_dark='#99d0d4',
        body_text_color_subdued='#99d0d4',
        body_text_color_subdued_dark='#99d0d4',
        background_fill_primary='#505a5b',
        background_fill_primary_dark='#505a5b',
        background_fill_secondary='#505a5b',
        background_fill_secondary_dark='#505a5b',
        border_color_accent='#99d0d4',
        border_color_accent_dark='#99d0d4',
        border_color_accent_subdued='#99d0d4',
        border_color_accent_subdued_dark='#99d0d4',
        border_color_primary='#99d0d4',
        border_color_primary_dark='#99d0d4',
        color_accent='#99d0d4',
        color_accent_soft='#99d0d4',
        color_accent_soft_dark='#99d0d4',
        link_text_color='#99d0d4',
        link_text_color_dark='#99d0d4',
        link_text_color_active='#99d0d4',
        link_text_color_active_dark='#99d0d4',
        link_text_color_hover='#99d0d4',
        link_text_color_hover_dark='#99d0d4',
        link_text_color_visited='#99d0d4',
        link_text_color_visited_dark='#99d0d4',
        block_background_fill='#505a5b',
        block_background_fill_dark='#505a5b',
        block_border_color='#99d0d4',
        block_border_color_dark='#99d0d4',
        block_info_text_color='#99d0d4',
        block_info_text_color_dark='#99d0d4',
        block_label_background_fill='#505a5b',
        block_label_background_fill_dark='#505a5b',
        block_label_border_color='#505a5b',
        block_label_border_color_dark='#505a5b',
        block_label_text_color='#99d0d4',
        block_label_text_color_dark='#99d0d4',
        block_title_background_fill='#505a5b',
        block_title_background_fill_dark='#505a5b',
        block_title_text_color='#99d0d4',
        block_title_text_color_dark='#99d0d4',
        panel_background_fill='#505a5b',
        panel_background_fill_dark='#505a5b',
        panel_border_color='#99d0d4',
        panel_border_color_dark='#99d0d4',
        chatbot_code_background_color='#505a5b',
        chatbot_code_background_color_dark='#505a5b',
        checkbox_background_color='#505a5b',
        checkbox_background_color_dark='#505a5b',
        checkbox_background_color_focus='#505a5b',
        checkbox_background_color_focus_dark='#505a5b',
        checkbox_background_color_hover='#505a5b',
        checkbox_background_color_hover_dark='#505a5b',
        checkbox_background_color_selected='#505a5b',
        checkbox_background_color_selected_dark='#505a5b',
        checkbox_label_text_color='#99d0d4',
        checkbox_label_text_color_dark='#99d0d4',
        checkbox_border_color='#99d0d4',
        checkbox_border_color_dark='#99d0d4',
        ...
        input_placeholder_color_dark='#95afb2',
        slider_color='#99d0d4',
        slider_color_dark='#99d0d4',
        stat_background_fill='#505a5b',
        stat_background_fill_dark='#505a5b',
        button_border_width='2px',
        button_border_width_dark='2px',
        button_primary_background_fill='#505a5b',
        button_primary_background_fill_dark='#505a5b',
        button_primary_background_fill_hover='#505a5b',
        button_primary_background_fill_hover_dark='#505a5b',
        button_primary_border_color='#99d0d4',
        button_primary_border_color_dark='#99d0d4',
        button_primary_text_color='#99d0d4',
        button_primary_text_color_dark='#99d0d4',
        button_secondary_background_fill='#505a5b',
        button_secondary_background_fill_dark='#505a5b',
        button_secondary_background_fill_hover='#505a5b',
        button_secondary_background_fill_hover_dark='#505a5b',
        button_secondary_border_color='#99d0d4',
        button_secondary_border_color_dark='#99d0d4',
